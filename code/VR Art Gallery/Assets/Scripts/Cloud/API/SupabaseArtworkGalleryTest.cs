...
/// </summary>
public class SupabaseArtworkGalleryTest : MonoBehaviour
{
    ...
    private async void Start()
    {
        ...
        try
        {
            ...
            var firstGallery = new GalleryData
            {
                ...
                artwork_ids = new List<int> { 67, 69, 180},
                artwork_map = new Dictionary<int, int> { { 0, 67 }, { 1, 69 }, { 2, 180 } }
            };
            ...
            var secondGallery = new GalleryData
            {
                ...
                description = testGalleryDescription,
                owner_id    = testOwnerId,
                artwork_ids = new List<int> { 179, 180 },
                artwork_map = new Dictionary<int, int> { { 0, 179 }, { 1, 180 } }
            };
            ...
            // Without thumbnails
            var artworkPaths = await _repository.GetArtworkPaths(createdFirst.id, 9, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (no thumbnails): {artworkPaths.Count} slot(s) returned");
            foreach (var entry in artworkPaths)
            {
                (string imageUrl, string thumbUrl) = entry.Value;
                Debug.Log($"  - slot {entry.Key} => image_url: \"{imageUrl}\", thumbnail_url: {thumbUrl ?? "null (expected)"}");
            }

            // With thumbnails
            var artworkPathsWithThumbs = await _repository.GetArtworkPaths(createdFirst.id, 9, true);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (with thumbnails): {artworkPathsWithThumbs.Count} slot(s) returned");
            foreach (var entry in artworkPathsWithThumbs)
            {
                (string imageUrl, string thumbUrl) = entry.Value;
                Debug.Log($"  - slot {entry.Key} => image_url: \"{imageUrl}\", thumbnail_url: \"{thumbUrl}\"");
            }

            // Limit to 1
            var artworkPathsLimited = await _repository.GetArtworkPaths(createdFirst.id, 1, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (maxArtWork=1): count = {artworkPathsLimited.Count} (should be 1)");

            // Fetch all with -1
            var artworkPathsAll = await _repository.GetArtworkPaths(createdFirst.id, -1, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (maxArtWork=-1, all): count = {artworkPathsAll.Count}");
    ...
    /// <summary>
    /// Tests GetArtworkPaths on a live gallery — creates a temp gallery, fetches paths
    /// with and without thumbnails across multiple maxArtWork limits, then cleans up.
    /// </summary>
    public async void ManualTestGetArtworkPaths()
    {
        Debug.Log("[SupabaseGalleryTest] Manual GetArtworkPaths test initiated...");
        try
        {
            _repository = await SupabaseGalleryRepository.CreateAsync();

            // Use artwork IDs that are known to exist in your DB
            var gallery = new GalleryData
            {
                name        = $"ArtworkPaths Test - {System.DateTime.Now:yyyy-MM-dd HH:mm:ss}",
                description = "GetArtworkPaths test gallery",
                owner_id    = testOwnerId,
            ...
            var created = await _repository.CreateGalleryAsync(gallery);
            Debug.Log($"[SupabaseGalleryTest] Created temp gallery ID: {created.id}");

            // ── Test 1: Without thumbnails, max 9 ────────────────────────────
            var pathsNoThumbs = await _repository.GetArtworkPaths(created.id, 9, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (no thumbnails): {pathsNoThumbs.Count} slot(s) returned");
            foreach (var entry in pathsNoThumbs)
            {
                (string imageUrl, string thumbUrl) = entry.Value;
                Debug.Log($"  - slot {entry.Key} => image_url: \"{imageUrl}\", thumbnail_url: {thumbUrl ?? "null (expected)"}");
            }
            ...
            var pathsWithThumbs = await _repository.GetArtworkPaths(created.id, 9, true);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (with thumbnails): {pathsWithThumbs.Count} slot(s) returned");
            foreach (var entry in pathsWithThumbs)
            {
                (string imageUrl, string thumbUrl) = entry.Value;
                Debug.Log($"  - slot {entry.Key} => image_url: \"{imageUrl}\", thumbnail_url: \"{thumbUrl}\"");
            }

            // ── Test 3: maxArtWork = 1 (limit respected) ─────────────────────
            var pathsLimited = await _repository.GetArtworkPaths(created.id, 1, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (maxArtWork=1): count = {pathsLimited.Count} (should be 1)");

            // ── Test 4: maxArtWork = -1 (fetch all) ──────────────────────────
            var pathsAll = await _repository.GetArtworkPaths(created.id, -1, false);
            Debug.Log($"[SupabaseGalleryTest] GetArtworkPaths (maxArtWork=-1, all): count = {pathsAll.Count} (should be 3)");

            // ── Cleanup ───────────────────────────────────────────────────────
            await _repository.DeleteGalleryAsync(created.id);
            Debug.Log($"[SupabaseGalleryTest] Cleaned up gallery ID: {created.id}");
            Debug.Log("[SupabaseGalleryTest] ManualTestGetArtworkPaths PASSED!");
        }
        catch (System.Exception ex)
        {
            Debug.LogError($"[SupabaseGalleryTest] ManualTestGetArtworkPaths FAILED: {ex.Message}");
            Debug.LogError($"[SupabaseGalleryTest] Stack Trace: {ex.StackTrace}");
        }
    }
