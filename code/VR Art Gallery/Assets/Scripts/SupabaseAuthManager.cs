...
public class SupabaseAuthManager : MonoBehaviour
{
    ...
    private IEnumerator RegisterAndCreateProfile(string email, string password, string username)
    {
        ...
        yield return PostJson(
            ...
            onError: (code, body) =>
            {
                ...
                // If user already exists, try login instead (dev convenience)
                if (body != null && body.Contains("user_already_exists"))
                {
                    Debug.Log("[SupabaseAuthManager] User exists, falling back to SignIn...");
                    StartCoroutine(LoginAndEnsureProfile(email, password));
                }
            }
        );
    ...
    private IEnumerator LoginAndEnsureProfile(string email, string password)
    {
        ...
        yield return UpsertArtistsRow(username: testUsername, includeCreatedAt: false);
    }
    ...
    private IEnumerator UpsertArtistsRow(string username, bool includeCreatedAt)
    {
        ...
        // user_id (text PK), created_at (timestamptz), managed_gallery (json[]), gallery_access (json[]), username (text)
        ...
        sb.Append(",\"managed_gallery\":[]");
        sb.Append(",\"gallery_access\":[]");
